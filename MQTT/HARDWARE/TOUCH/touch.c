...
//端序转换
uint32_t __reverseBytesUInt32(uint32_t value) {
 ...
 * @retvalue   :0-fail,1-success
******************************************************************************/ 
//连续读取触摸屏坐标两次,并且这两次的偏差不能超过ERR_ RANGE
...
#define SAVE_ADDR_BASE  address(0,1,0)
/*****************************************************************************
...
void TP_Save_Adjdata(void)
{
	...
	memset(w25q_buf,0,sizeof(w25q_buf));
	
	//保存校正结果!		   							  
	temp=tp_dev.xfac*100000000;//保存x校正因素
	strncpy(w25q_buf,(char*)&temp,4);
	temp=tp_dev.yfac*100000000;//保存y校正因素 	
	strncpy(w25q_buf+4,(char*)&temp,4);
	
	//保存x偏移量
	strncpy(w25q_buf+8,(char*)&tp_dev.xoff,2);
	//保存y偏移量
	strncpy(w25q_buf+10,(char*)&tp_dev.yoff,2);
	//保存触屏类型
	w25q_buf[12] = tp_dev.touchtype;
	//标记校准过了
	w25q_buf[13] = 0X0A;

	//W25Q 0块1扇区擦除
	w25qxx_earase_sector(SAVE_ADDR_BASE);
	//校准数据写入W25Q
	w25qxx_pageProgram(SAVE_ADDR_BASE,(uint8_t*)w25q_buf,14);
}
...
u8 TP_Get_Adjdata(void)
{
	...
	memset(w25q_buf,0,sizeof(w25q_buf));
	w25qxx_read_data(SAVE_ADDR_BASE,(uint8_t*)w25q_buf, 14);

	tempfac=w25q_buf[13];//读取标记字,看是否校准过！ 

	if(tempfac==0X0A)//触摸屏已经校准过了			   
	{   
		tempfac = 0;		
		strncpy((char*)&tempfac,w25q_buf,4);
		tp_dev.xfac=(float)tempfac/100000000;//得到x校准参数
		
		tempfac = 0;
		strncpy((char*)&tempfac,w25q_buf+4,4);	
		tp_dev.yfac=(float)tempfac/100000000;//得到y校准参数
	    
		//得到x偏移量
		tp_dev.xoff = 0;
		strncpy((char*)&tp_dev.xoff,w25q_buf+8,2);
		
 	    //得到y偏移量
		tempfac = 0;
		strncpy((char*)&tp_dev.yoff,w25q_buf+10,2);
		
		tp_dev.touchtype=w25q_buf[12];//读取触屏类型标记
		...
		{	
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					delay_ms(1000);
					TP_Save_Adjdata();
					LCD_ShowString(35,130, 16,"Touch Save Adjdata OK!",1);//保存校准参数
 					LCD_Clear(WHITE);//清屏   
					return;//校正完成				 
			}
		}
...
{			    		   
	...
	GPIO_InitTypeDef GPIO_InitStructure;	//GPIOD
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD, ENABLE);//使能GPIOD时钟

    //GPIOD1,14初始化设置
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1;//PD1设置为上拉输入
	...
	w25qxx_spi_init();//w25qxx_SPI--Falsh初始化
